...
export default function Header({user, showFilters, filterName, setFilterName, cuisine, setCuisine, price, setPrice, rating, setRating, setReefetch, setShowMap, setShowFilters}) {
        ...
        <AppBar position="static" sx={!showFilters ? {backgroundColor: 'transparent', zIndex: "5, 0 .5rem 1rem rgba(0,0,0,.15)!important"} : {backgroundColor: "#fffdfa", zIndex: "0 .5rem 1rem rgba(0,0,0,.15)!important"}}>
            <Toolbar>
                <Grid container sx={{alignItems: "center"}}>
                    ...
                    <Grid item sm={3}><Typography variant='h4' color='black' onClick={() => {setShowMap(false); setShowFilters(false);}}>Munch-Pocket</Typography></Grid>
                    <Grid item sm={1}></Grid>
                    {user && (<>

                        <Grid item sm={1.5}><Button sx={{backgroundColor: '#fbd867'}}><Typography sx={{fontWeight: "bold"}} variant='overline'
                        ...
                        <Grid item sm={3} sx={{display: 'flex', justifyContent: 'flex-end'}}><Button
                            startIcon={<ReceiptIcon/>} sx={{backgroundColor: '#fbd867',textAlign: 'center'}}><Typography
                        ...
                        {showFilters && (
                            <>
                                ...
                                <Grid item sm={2} sx={{}}>
                                    <Button sx={{backgroundColor: '#fbd867'}} text='Filter' onClick={() => {setReefetch(true);}}/>
                                </Grid>
                                <Grid item sm={12} sx={{height: '2vh'}}/>
                            </>
                        )}
